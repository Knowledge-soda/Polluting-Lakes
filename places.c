...
int init_places(Places *places, int w, int h, int screen_w, int screen_h){
    ...
    for (x = w - 1;x >= 0;x--){
        tmp = f + (w * y) + x;
        tmp -> x = CLR_EDGE + PLACE_SIZE / 2 + x * Dx;
        tmp -> y = CLR_EDGE + PLACE_SIZE / 2 + y * Dy;
...
int blit_places(Places *places, SDL_Renderer *render){
    ...
    for (x = 0;x < w;x++){
        cur = map + y * w + x;
        if (cur -> right) SDL_RenderDrawLine(render,
                cur -> x, cur -> y, cur -> right -> x, cur -> right -> y);
        if (cur -> down) SDL_RenderDrawLine(render,
                cur -> x, cur -> y, cur -> down -> x, cur -> down -> y);
        if (cur -> r_diag) SDL_RenderDrawLine(render,
                cur -> x, cur -> y, cur -> r_diag -> x, cur -> r_diag -> y);
        if (cur -> l_diag) SDL_RenderDrawLine(render,
